...
import (
	...
	"github.com/dscontrak/go-gorm-mux/api/app/handler"
	"github.com/dscontrak/go-gorm-mux/api/app/model"
...
func (a *App) Initialize(config *config.Config) {
	...
	// For SQLite
	dbURI := fmt.Sprintf("%s", config.DB.Name)
	fmt.Println("dbURI=" + dbURI + ", config:" + config.DB.Dialect)
	db, err := gorm.Open(config.DB.Dialect, dbURI)

	if err != nil {
		log.Fatal("Could not connect database. Error: ", err)
	}
